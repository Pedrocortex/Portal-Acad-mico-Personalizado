...
def valida_nome(nome):
    ...
    # 1. Limpa espaços no início e no fim
    nome_limpo = limpar_texto(nome)

    # 2. Verifica se o nome, após a limpeza, está vazio
    if not nome_limpo:
        st.error('O nome não pode estar vazio.')
        return False

    if contar_caracteres(nome_limpo) > 255:
        st.error('O nome não pode passar de 255 caracteres.')
        return False

    espacos = nome_limpo.count(' ')
    if espacos < 1:
