...
def modulo_disciplinas():
    ...
    with st.expander('➕ Nova Disciplina'):
        ...
        limite_falta = st.number_input('Limite de Faltas', min_value=0, step=1) 
        faltas_atuais = st.number_input('Faltas Atuais', min_value=0, step=1)
        ...
        if st.button('Salvar Disciplina'):
            if not util.valida_nomeD(nome_d):
                return
            api.api_post('disciplinas', {'nome': nome_d, 'professores_id': opcoes_p[p_escolhido], 'carga_horaria': carga_horaria, 
                                        'limite_falta': limite_falta, 'faltas': faltas_atuais, 'nota': nota, 'dia': dia, 'horario': horario.strftime('%H:%M')})
            st.rerun()
...
def modulo_dashboard():
    ...
    if not discs:
        st.info('Cadastre disciplinas para visualizar seu desempenho.')
        return
    ...
    fig = px.bar(
        ...
        x='nome',
        y='nota',
        title='Minhas Notas por Disciplina',
        text_auto=True
    )
